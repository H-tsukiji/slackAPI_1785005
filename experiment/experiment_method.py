...
def ranking_late(L_result, S_result, cos_result, weight_value):
    sum_data = ranking(L_result, S_result, cos_result,1.0)
    usernum = len(L_result)
    a = weight_value
    print("リーダ性＋サポート性/全ての合計値")
    #ループごとにユーザのスコア算出
    for (i,user) in enumerate(L_result):
        print(user)
        for (j,targetuser) in enumerate(L_result):
            ...
            if i == j:
                continue
            sum_score = L_result[targetuser]+S_result[targetuser][user]
            score = (a* (sum_score/sum_data[user]) )+((1-a)*(cos_result[i][j]))
            print(targetuser,score)
...
if __name__ == '__main__':
    ...
    ranking_result = ranking(L_result,S_result,cos_result,weight_value)
    #ranking_log(L_result,S_result,cos_result,weight_value)
    ranking_late(L_result,S_result,cos_result,weight_value)
